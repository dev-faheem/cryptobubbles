...
import { useToggleStore } from "../lib/store/useToggleStore";
...
export default function NavigationBar({ onClick }: NavigationBarProps) {
  ...
  return (
    <>
      <ul className="xl:hidden flex gap-2 text-xs ">
        <li>
          <button
            onClick={() => setActiveTab("bubbles")}
            className={clsx(
              "gap-1 inline-flex items-center justify-center text-center cursor-pointer text-white",
              activeTab === "bubbles" ? "bg-blue-500" : "bg-transparent",
              "hover:bg-[#ffffff1f] border-zinc-800 xl:rounded-full p-[10px] m-0 transition-colors duration-[400ms]"
            )}
            title="Go to Table"
          >
            <svg viewBox="0 0 24 24" className="fill-current pointer-events-none h-6 w-6">
        ...
        <li>
          <button
            ...
            className={clsx(
              "gap-1 inline-flex items-center justify-center text-center cursor-pointer text-white",
              activeTab === "settings" ? "bg-blue-500" : "bg-transparent",
              "hover:bg-[#ffffff1f] border-zinc-800 xl:rounded-full p-[10px] m-0 transition-colors duration-[400ms]"
            )}
            title="Settings"
          >
      ...
      <ul className="hidden xl:flex gap-2 text-xs">
        <li>
          <button
            onClick={handleScrollToTable}
            className={clsx(
              "gap-1 inline-flex items-center justify-center text-center cursor-pointer text-white",
              "xl:bg-[#ffffff1f]  border-zinc-800",
              "xl:rounded-full p-[10px] m-0 transition-colors duration-[400ms]"
            )}
            title="Go to Table"
          >
            <svg viewBox="0 -960 960 960" className="fill-current pointer-events-none h-6 w-6">
